The Future I’m Building — Locked Story

Global identity

This is a fluid fixed-viewport web story about Jovan Pahal.

It is not a numbered slide presentation.

It covers the original capstone topics across 34 story beats.

The story is about Jovan first. Oral surgery is revealed later as the career where his existing interests and skills can come together.

Target reading time: 10–12 minutes.

No visible scene or slide counter.

Loader

Final loader text:

JOVAN PAHAL

Act cursor words

Act 1: PROJECTS

Act 2: SOUND

Act 3: PERSPECTIVE

Act 4: ORIGIN

Act 5: PRECISION

Act 6: BUILD

Act 7: OPPORTUNITY

Act 8: DIRECTION

Beat 1 — Opening

Corner labels:

Top left:

CREATIVITY

Top right:

SKILLS

Bottom left:

AMBITION

Bottom right:

PROJECTS

Central title:

who am i

The central title must be lowercase.

The corner words must be uppercase.

There is no sentence, subtitle, name, object, hand, eye, description, button, or counter on this beat.

Beat 2 — Opening interlude

I love building, learning, creating, and trying things.

Beat 3 — GitHub / newspaper scene

Masthead:

PROJECT LOG

Headline:

THE THINGS I TRY LIVE HERE

Body:

GitHub is where I keep the websites, applications, experiments, and side projects I build. The contribution squares do not show that every idea succeeded. They show how often I returned to making something. When an idea does not work, I can try again, change direction, or carry what I learned into the next project.

Caption:

My projects do not all have to lead to the same result. I build them because technology lets me turn almost any idea into something real.

Asset:

assets/personal/projects/github-contributions.png

Beat 4 — Projects interlude

Not every idea works.

That does not make the attempt useless.

Beat 5 — Music introduction

Being able to play an idea is not the same as knowing what the song needs.

Beat 6 — Music production

Paragraph 1:

I can learn almost anything on a few instruments, but that is only one part of production. Knowing the melody does not tell me where it belongs, what should surround it, or what the song needs next.

Paragraph 2:

Working with somebody else also means I cannot replace everything they made just because I think another version sounds better. I have to understand what is already working, preserve what matters, and build onto it.

Paragraph 3:

Sometimes I can spend a week developing one idea and a quick cookup still turns out better. Production is teaching me when to keep pushing an idea and when to leave it behind and try another direction.

Assets:

* assets/personal/music/ableton-screenshot.png
* assets/personal/music/production-footage-poster.png
* assets/personal/music/production-footage.mp4

Beat 7 — Progress interlude

Progress can be slow enough that you barely notice it.

Beat 8 — Music opportunity

Paragraph 1:

My uncle is a producer. If I become as good as I want to be, that gives me a real door into that world.

Paragraph 2:

But having the door open does not make up for not being good enough to walk through it.

Paragraph 3:

The beats I have now are not the final result. They are proof that I am improving, even when it takes many attempts before the difference becomes obvious.

Beat 9 — Photography introduction

You can always delete a bad picture, but you can’t go back and take one you never tried to get.

Beat 10 — Japan photograph 1

Title:

THE PICTURE INSIDE THE PICTURE

Text:

A photo can be hiding inside another photo. In my uncle’s sunglasses, the reflection of me holding the camera became the real subject. The picture was already there. I just had to notice it.

Asset:

assets/personal/photography/japan-01-sunglasses.png

Beat 11 — Japan photograph 2

Title:

A SKY FULL OF SEPARATE LIVES

Text:

From the ground, a plane looks like one object. Inside it are hundreds of people, each on a trip that feels just as important and exciting to them as mine does to me. And that is only one plane.

Asset:

assets/personal/photography/japan-02-plane.png

Beat 12 — Japan photograph 3

Title:

OLD MEETS NEW

Text:

The temple and Skytree share the same frame. Building something new does not mean throwing away everything old. You can keep the lessons that still matter and use them to build something bigger.

Asset:

assets/personal/photography/japan-03-temple-skytree.png

Beat 13 — Japan photograph 4

Title:

BUILD AROUND THE OBSTACLE

Text:

Osaka did not wait for the river to disappear before building around it. Some problems are not going away, so the better move is to understand them and work around them.

Asset:

assets/personal/photography/japan-04-osaka-river.png

Beat 14 — Japan photograph 5

Title:

ROOM INSIDE THE NOISE

Text:

In one of the busiest cities in the world, I found a quiet patch of cherry blossoms beside a train station. Even when life feels completely full, there is usually still room for what matters. You just have to find it.

Asset:

assets/personal/photography/japan-05-cherry-blossoms.png

Beat 15 — Photography ending

Perspective changes what is available to notice.

Beat 16 — Origin interlude

THE IDEA STAYED.

THE VISION GOT BIGGER.

Beat 17 — Kindergarten archive

Headline:

I WANTED TO BECOME A DENTIST BEFORE I UNDERSTOOD WHAT THAT ACTUALLY MEANT.

Context:

The original idea was simple. As I learned more about healthcare, technology, business, and the way I like to work, the idea became more specific.

Assets:

* assets/personal/origin/kindergarten-video-poster.png
* assets/personal/origin/kindergarten-video.mp4

Beat 18 — Oral-surgery reveal

Title:

ORAL AND MAXILLOFACIAL SURGERY

Main line:

The career where the skills I am building now can come together.

Context:

It gives me a place to apply precision, technology, responsibility, teamwork, business thinking, and the desire to keep building more.

Beat 19 — Why it fits

Main line:

It lets me take what I am already working on and apply it to one career.

Supporting ideas:

* Advanced skill
* Healthcare
* Helping people
* Technology
* Ownership
* Long-term options

Beat 20 — Dental-scan introduction

I took an opportunity to try unfamiliar technology and turned it into a project of my own.

Beat 21 — Rotating dental model

Paragraph 1:

I set up the scanner software, scanned my little brother’s teeth with an intraoral scanner, and exported the result as an STL model.

Paragraph 2:

I did it for fun, but it connected my interest in technology and side projects directly to dentistry.

Moving words:

* INTRAORAL SCAN
* DIGITAL IMPRESSION
* LOWER ARCH
* 3D MODEL
* ROTATE
* INSPECT
* EXPORT

Asset:

assets/dental/lower-teeth.stl

Only the lower-teeth model should be used.

Beat 22 — Healthcare and ownership

Main line:

I want a healthcare path where I can help people and still build something of my own.

Context:

A dental practice is not only a place where treatment happens. It is also a team, a system, a patient experience, and a business.

Beat 23 — Practice system

Main line:

Behind every appointment is a much larger system.

Supporting ideas:

* Front desk
* Scheduling
* Assistants
* Hygienists
* Equipment
* X-rays
* Patient flow
* Decisions
* Dentist moving between rooms

Beat 24 — Building project

Headline:

WATCHING AN IDEA BECOME A REAL BUILDING

Context:

I watched my dad’s project from when he acquired the land through planning, blueprints, and the beginning of construction. I built the website and am helping document the process.

Reflection:

It showed me that building something larger is not one decision. It is a long sequence of planning, problem-solving, patience, and continuing to add onto what already exists.

Assets:

* assets/personal/projects/building-blueprints-01.png
* assets/personal/projects/building-blueprints-02.png
* assets/personal/projects/photo-with-dad.png

Beat 25 — Practice expansion

Main line:

I like to keep building onto what I create.

Context:

The same thinking could eventually apply to dentistry: improving one practice, strengthening its systems, adding rooms or people, and eventually expanding into something larger.

Beat 26 — Tools

AI does not replace my thinking.

Asset:

assets/generated/tools/ai-process.png

Beat 27 — How I use AI

Line 1:

AI helps me research, organize, test, and build faster.

Line 2:

It cannot decide what I care about or what I am trying to make.

Line 3:

I still choose the idea, make the important decisions, and decide what the final result should become.

Beat 28 — Connections

Quote:

Connections do not replace skill.

They create opportunities for skill.

Context:

My uncle being a producer gives me one possible door. Growing up around dentistry gives me another. Neither opportunity matters unless I develop the ability to actually use it.

Asset:

assets/generated/connections/opportunity-door.png

Beat 29 — The parts still continue

Main line:

Oral surgery can become the anchor without becoming the only thing I do.

Context:

I can keep building, producing, taking photographs, and experimenting with technology while working toward one serious career.

Reuse these visuals:

* GitHub contributions
* music-production footage/poster
* Japan photography
* building blueprints
* lower-teeth model/render

Beat 30 — Integration

I do not want to give up one side of myself to build another.

Beat 31 — The path

THE PATH IS LONG.

Asset:

assets/generated/ending/forward-path.png

Beat 32 — The next version

Opening:

That does not mean everything else has to wait.

Then reveal these lines separately:

Keep building projects.

Keep producing even when progress is slow.

Keep finding photographs worth taking.

Keep learning more about dentistry, technology, ownership, and how larger systems work.

Keep becoming good enough to use the opportunities that appear.

Beat 33 — Sources

Reveal these sources separately:

* Canadian Association of Oral and Maxillofacial Surgeons
* Canadian Dental Association
* Digital dentistry and intraoral-scanning research
* Personal photographs, projects, and experience

The sources must fade away before Beat 34.

Beat 34 — Ending

Final quote:

I do not have to choose one part of myself to build the future I want.

Final credit:

Website built by Jovan Pahal
